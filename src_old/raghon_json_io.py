# jsonで読み込み→jsonで出力することができるようになった
# リストにも対応してる。出力したjsonを読み込んで再利用することも可能
...
def main():
    ...
    with open("input/sample.json", "r", encoding='utf-8') as f:
        ...
        logger.info(f"dict_json: {dict_json}")
        logger.info(f"dict_json['query']: {dict_json['query']}")
    
    # ワークフローを複数質問で回す
    answer_list = []
    for seed_query in dict_json['query']:
        logger.info(f"seed_query: {seed_query}")
        initial_state = State(query=seed_query)
        result = compiled.invoke(initial_state)
        logger.info(f"result: {result}")
        answer_list.append(result)

    # jsonとして出力。リストに入れてからjson化したので、読み込みして再利用可能
    answer_dict = {"answer": answer_list}
